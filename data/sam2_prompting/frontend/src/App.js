...
function App() {
  ...
  const fetchData = (navigation = 'current') => {
    fetch(`${API_URL}/data?navigation=${navigation}`)
      ...
      .then((json) => {
        ...
        console.log('JSON w/ keys:', Object.keys(json));
        console.log('JSON content:', json.frame_idx);
        if (json && json.frame) {
          const img = new Image();
          img.onload = () => {
            ...
            setSelectedVideoName(json.path_v || "");
            setSelectedFrameIdx(json.frame_idx >= 0 ? json.frame_idx : -1);
  ...
  return (
    <div className="container">
      {data ? (
        <div className="annotation-ui" style={{ maxHeight: '100vh', overflow: 'auto' }}>
          ...
          <div className="info-panel" style={{ maxHeight: '300px', overflow: 'auto' }}>
            <p>
              Video: {selectedVideoName || "Loading..."} | Frame: {selectedFrameIdx !== -1 ? selectedFrameIdx : "Loading..."}
            </p>
            <p>
